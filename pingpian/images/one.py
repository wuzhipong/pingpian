...
#print '正在处理数据请稍等...'
def mysql_conn():
	db = MySQLdb.connect("127.0.0.1","root","root","evaluationsystem",charset='utf8')
	return db
...
def to_other_file(img_name_list,img_dir_list, type):
    ...
    for img in img_dir_list:
        ...
        #r_img.save(r'C:\\pingpian\\static\\'+imgsrc+'.png')#使用PIL会出现异常，原因未知
        cv2.imwrite(r'C:\\pingpian\\ppsystem\\static\\image\\original\\'+imgsrc+'.png',r_img)
        datastr = imgsrc+'.png'
        db = mysql_conn()
        cursor = db.cursor()
        #将数据插入数据库
        sql = "INSERT INTO ppsystem_imagedata(name,se_original_img,\
               ...
               ,str(r'C:\\pingpian\\ppsystem\\static\\image\\original\\'+imgsrc+'.png'))
        try:
            cursor.execute(sql)
            db.commit()
        except:
